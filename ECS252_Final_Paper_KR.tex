...
특히 본 연구에서는 CSE-CIC-IDS2018을 binary IDS task로 학습하면서도 원래 attack label을 보존해 post-hoc attack-type analysis를 수행할 수 있다는 점이 중요하다.

IDS modeling에 대한 prior work는 본 연구의 model set에 동기를 부여한다.
Logistic Regression은 simple linear baseline으로 사용되며, Random Forest는 many decision-tree predictor를 ensemble로 결합하기 때문에 standard traditional machine learning baseline으로 사용된다 [3].
...
더 구체적으로 Rosay, Carlier, Leroux는 CICIDS2017 dataset에서 MLP 기반 intrusion detection system을 제안했고, 잘 설계된 MLP가 traditional machine learning technique과 경쟁할 수 있음을 보였다 [5].
Song, Hyun, Cheong은 network intrusion detection을 위한 autoencoder를 분석하면서 model capacity, latent size, thresholding 같은 설계 선택이 IDS performance에 큰 영향을 줄 수 있음을 보였다 [6].
이는 MLP 기반 neural model과 autoencoder 기반 representation-learning model을 포함하는 본 연구의 비교 구성을 뒷받침한다.
동시에 prior work는 neural architecture를 추가하는 것만으로는 새로운 contribution이 되기 어렵다는 점도 보여준다.
...
Saito and Rehmsmeier는 imbalanced classification problem에서 precision-recall analysis가 ROC plot보다 더 informative하다는 점을 보였다 [7].
이 점은 IDS에서 중요하다.
False negative는 system이 malicious traffic을 flag하지 못했다는 의미이기 때문이다.
따라서 본 연구는 recall, false-negative rate, attack-type detection rate를 부가적 진단 지표가 아니라 핵심 security metric으로 다룬다.

따라서 이 프로젝트는 existing work 안에 위치하지만 더 좁고 security-centered question을 묻는다.
나는 neural model이 traditional machine learning보다 단순히 IDS를 더 잘한다고 주장하지 않는다.
대신 Logistic Regression, Random Forest, Shallow MLP, Lightweight PyTorch MLP, Deep MLP, Autoencoder + MLP를 같은 flow-level pipeline에서 비교해 각 모델이 어떤 attack category를 놓치는지 식별한다.
따라서 neural 및 traditional baseline은 새로운 best IDS architecture를 주장하기 위한 것이 아니라, common model-comparison metric이 attack-specific failure mode를 가리는지 확인하기 위한 것이다.
이 framing은 최종 실험 목표와도 맞다.
Neural model은 precision이나 F1-score를 개선할 수 있지만, 더 중요한 security question은 DoS/DDoS, brute force, botnet, infiltration, web attack category 전반에서 missed attack을 실제로 줄이는지이다.
Related work는 세 가지 방식으로 이 프로젝트를 지지한다.
CICIDS dataset은 labeled flow-level setting을 제공하고, prior ML/DL IDS work는 model comparison을 정당화하며, imbalanced-evaluation research는 aggregate metric을 보완하기 위해 attack-type recall과 false-negative analysis를 사용해야 하는 이유를 제공한다.
...
이 표는 본 연구의 central tension을 보여준다.
...
Logistic Regression은 가장 높은 recall(0.528)과 가장 낮은 false-negative rate(0.472)를 보였고, 1,197개 attack을 놓쳤다.
Precision은 가장 낮았지만, missed malicious flow를 최소화하는 것이 우선이라면 이번 실험에서 가장 안전한 모델이다.

\paragraph{Random Forest.}
Random Forest는 precision을 0.649까지 높였지만 recall은 0.471로 낮아졌고 1,342개 attack을 놓쳤다.
Binary level에서는 Logistic Regression보다 더 selective했지만, 뒤의 attack-type result에서는 Botnet과 Web Attack에서 강점을 보인다.

\paragraph{Shallow MLP.}
Shallow MLP는 neural model 중 recall이 가장 높았고, 0.524 recall과 1,209개 missed attack을 기록했다.
Logistic Regression과 비슷한 recall profile을 보이면서 precision과 F1-score는 더 높아 가장 균형 잡힌 neural baseline이었다.

\paragraph{Lightweight PyTorch MLP.}
Two-hidden-layer PyTorch MLP는 Logistic Regression보다 precision과 F1-score를 높였지만, recall은 0.507로 낮아졌고 false negative는 1,252개로 증가했다.
따라서 missed attack을 명확히 줄이기보다는 moderate precision-recall tradeoff를 제공한다.

\paragraph{Deep MLP.}
Deep MLP는 가장 높은 precision(0.747)과 F1-score(0.574)를 보였지만, false-negative rate도 0.533으로 가장 높았고 1,354개 attack을 놓쳤다.
즉 malicious로 예측할 때는 더 정확하지만, 더 많은 실제 attack을 탐지하지 못한 가장 selective한 모델이다.

\paragraph{Autoencoder + MLP.}
Autoencoder + MLP는 Deep MLP와 유사하게 높은 precision(0.723), 높은 F1-score(0.570), 높은 false-negative rate(0.529)를 보였다.
Learned representation은 selectivity를 높였지만 IDS recall problem을 해결하지는 못했다.
...
이 논문이 말하고자 하는 핵심 메시지는 IDS model evaluation이 aggregate score에서 멈추면 안 된다는 것이다.
Precision, recall, F1-score는 모델을 빠르게 비교하는 데 유용하지만, 어떤 attack type이 여전히 uncovered로 남는지는 충분히 보여주지 못한다.
이번 실험에서 neural model은 일부 aggregate metric을 개선했지만, 그것이 곧 더 넓은 security coverage를 의미하지는 않았다.
...
이 결과의 핵심은 단순하다.
모델을 바꾸면 IDS의 전체 점수만 바뀌는 것이 아니라, 어떤 attack을 놓치는지도 함께 바뀐다.
Shallow MLP와 PyTorch MLP는 Brute Force family에서 Random Forest와 비슷했고, MLP 계열 모델은 DoS/DDoS traffic에서도 좋은 성능을 보였다.
하지만 neural model이 모든 attack type에서 missed attack을 줄인 것은 아니었다.
오히려 전체 recall이 가장 높았던 모델은 Logistic Regression이었고, deeper neural model들은 더 많은 attack을 놓쳤다.

이 점이 중요한 이유는 평가 기준에 따라 결론이 달라지기 때문이다.
F1-score나 precision만 보면 Deep MLP가 가장 좋은 모델처럼 보인다.
Deep MLP는 malicious라고 예측한 경우에는 비교적 정확했다.
하지만 IDS에서는 “탐지한 alert가 얼마나 정확한가”만큼 “공격을 얼마나 놓치지 않는가”도 중요하다.
이 기준으로 보면 Logistic Regression과 Shallow MLP가 더 안전한 선택으로 보인다.
따라서 best model은 하나의 숫자로 정해지지 않는다.
어떤 보안 목표를 우선하느냐에 따라 좋은 모델이 달라진다.

\paragraph{Attack-type blind spots.}
Attack-type result는 왜 aggregate metric만으로 부족한지를 보여준다.
쉬운 attack에서 성능이 높으면 전체 점수는 좋아질 수 있다.
하지만 그와 동시에 stealthy하거나 ambiguous한 attack은 계속 놓칠 수 있다.
이번 결과에서도 DoS/DDoS와 FTP/SSH brute-force traffic은 대부분 잘 탐지되었다.
반면 Bot과 SQL Injection에서는 Random Forest가 특히 강했고, Infiltration은 모든 모델이 공통적으로 약했다.
특히 Infiltration은 test set에서 크기도 컸고, 전체 false negative의 대부분을 차지했다.
여섯 모델에서 Infiltration은 전체 false negative의 96.8\%에서 99.5\%를 차지했다.
따라서 aggregate score가 좋아 보여도 실제로는 가장 중요한 missed attack이 한 attack family에 집중될 수 있다.

\paragraph{Diagnostic value.}
이 분석은 attack-type-specific evaluation이 단순한 보조 설명이 아니라 diagnostic tool임을 보여준다.
모델별로 잘 잡는 attack이 다르면, 각 모델이 어떤 traffic pattern에 강한지 알 수 있다.
예를 들어 Random Forest가 Bot과 SQL Injection에서 강했다는 것은 일부 botnet 및 web attack behavior가 tabular flow feature의 threshold-style pattern으로 잘 포착될 수 있음을 시사한다.
반대로 모든 모델이 Infiltration에서 약했다는 것은 단일 모델의 문제가 아니라, 현재 feature, dataset, 또는 binary training objective 자체가 이 attack family를 충분히 설명하지 못했을 가능성을 보여준다.

\paragraph{Operational choice.}
운영 관점에서도 함의가 있다.
IDS를 배치할 때 단순히 average score가 가장 높은 모델을 고르는 것은 충분하지 않다.
False alarm을 줄이는 것이 가장 중요하다면 precision이 높은 Deep MLP나 Autoencoder + MLP가 매력적일 수 있다.
하지만 missed attack을 줄이는 것이 더 중요하다면 Logistic Regression이나 Shallow MLP가 더 나은 선택일 수 있다.
또한 Infiltration처럼 모든 모델이 공통적으로 놓치는 attack family가 있다면, 모델을 하나 더 추가하는 것보다 그 attack에 대한 data collection, threshold tuning, 또는 attack-family-specific training이 먼저 필요하다.
결국 운영 환경의 best model은 평균 점수가 아니라, 어떤 attack을 반드시 잡아야 하는지에 따라 선택되어야 한다.

\paragraph{Hybrid IDS design.}
이 결과는 future hybrid IDS 설계에도 연결된다.
모든 malicious traffic에 하나의 global decision rule을 적용하기보다, attack family별로 threshold를 다르게 두거나 여러 모델의 signal을 조합할 수 있다.
예를 들어 botnet이나 web attack에서는 Random Forest의 signal을 더 중요하게 보고, DoS/DDoS나 brute-force traffic에서는 MLP 계열 모델의 signal을 활용할 수 있다.
이런 방식은 “하나의 winner model”을 찾는 접근보다 더 유연하다.
각 모델이 잘 cover하는 attack type을 조합해 전체 IDS coverage를 넓히는 방향이기 때문이다.

\paragraph{Autoencoder caution.}
Autoencoder + MLP 결과도 조심스럽게 해석해야 한다.
Autoencoder는 high-dimensional flow feature를 compact representation으로 바꿀 수 있기 때문에 IDS 연구에서 자주 사용된다.
하지만 이번 실험에서는 Autoencoder + MLP가 Deep MLP와 비슷하게 동작했다.
Precision과 F1-score는 높았지만 recall은 낮았다.
이것은 autoencoder가 IDS에 쓸모없다는 뜻은 아니다.
다만 이번 설정에서는 representation learning을 추가해도 false-negative problem이 자동으로 해결되지는 않았다는 뜻이다.
따라서 더 깊은 모델이나 더 복잡한 representation을 사용하는 것만으로 security coverage가 좋아진다고 가정해서는 안 된다.
...
결국 이 논문의 메시지는 하나다.
IDS evaluation은 model-level leaderboard에서 멈추지 말고, attack-type-level coverage를 반드시 함께 평가해야 한다.
